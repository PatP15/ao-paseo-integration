...
func executionOperations() []operation {
	return []operation{
		...
		{
			method: http.MethodPost, path: "/api/v1/sessions/{sessionId}/execution-messages",
			id: "sendSessionExecutionMessage", tag: "execution",
			summary:    "Queue one free-form message for a session's remote agent",
			pathParams: []any{controllers.ExecutionSessionMessagePathParams{}},
			reqBody:    controllers.SendExecutionMessageRequest{},
			resps: []respUnit{
				{http.StatusAccepted, controllers.ExecutionSessionMessageResponse{}},
				{http.StatusBadRequest, envelope.APIError{}},
				{http.StatusNotFound, envelope.APIError{}},
				// Conflict is the session that runs on no execution host: there is
				// no remote agent for a message to reach.
				{http.StatusConflict, envelope.APIError{}},
				{http.StatusInternalServerError, envelope.APIError{}},
				{http.StatusNotImplemented, envelope.APIError{}},
			},
